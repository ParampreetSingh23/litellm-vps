...
vi.mock("@/components/liteadmin/LiteAdmin", () => ({
  default: () => <button>RAW Assistant</button>,
}));
...
describe("(dashboard) Layout", () => {
  ...
  it.each(["/ui/playground", "/ui/playground/"])(
    "hides RAW Assistant on %s and restores it after leaving Playground",
    async (pathname) => {
      const dashboard = () => (
        <AuthProvider>
          <Layout>
      ...
      rerender(dashboard());
      expect(screen.getByRole("button", { name: "RAW Assistant" })).toBeInTheDocument();
    },
  );
