...
const CostMath = ({ counters, detail }: { counters: CounterRow[]; detail: GuardrailUsageDetail }) => (
  <CalcPopover title="How this cost is calculated" formula="priced units × price per unit = cost, per counter">
    ...
    <p className="text-xs text-muted-foreground">Per-unit prices come from the cost map RAW ships with.</p>
    <UnpricedNote unpriced={detail.untracked_usage_units} />
  </CalcPopover>
);
    ...
    <section className="space-y-4" aria-label="Usage and cost">
      <div>
        ...
        <p className="mt-0.5 text-xs text-muted-foreground">
          Billable units the provider reported for this guardrail and what RAW priced them at
        </p>
      </div>
