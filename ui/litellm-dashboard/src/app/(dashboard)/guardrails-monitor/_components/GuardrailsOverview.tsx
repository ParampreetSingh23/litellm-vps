...
const providerColors: Record<string, string> = {
  ...
  RAW: "bg-brand/10 text-brand border-brand/20 dark:bg-brand/25 dark:text-foreground dark:border-brand/40",
      ...
      cell: ({ row }) => (
        <span
          className={`inline-flex items-center px-2 py-0.5 text-xs font-medium rounded border ${
            providerColors[row.original.provider === "LiteLLM" ? "RAW" : row.original.provider] ?? providerColors.Custom
          }`}
        >
          {row.original.provider === "LiteLLM" ? "RAW" : row.original.provider}
        </span>
      ),
    },
    {
