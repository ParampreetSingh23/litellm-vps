...
describe("CreateMCPServer", () => {
  ...
  describe("when HTTP transport is selected", () => {
    ...
    it("should warn that RAW auth is disabled when True Passthrough is selected", async () => {
      await selectHttpTransport();

      await selectOption("Authentication", "True Passthrough (no RAW auth)");

      await waitFor(() => {
        expect(screen.getByText("True Passthrough disables RAW authentication for this server")).toBeInTheDocument();
      });
    });
    ...
    it("should not show the True Passthrough warning when OAuth Delegate is selected", async () => {
      ...
      expect(
        screen.queryByText("True Passthrough disables RAW authentication for this server"),
      ).not.toBeInTheDocument();
    });

    it.each([["True Passthrough (no RAW auth)"], ["OAuth Delegate (client-supplied upstream token)"]])(
    ...
    it.each([
      ["true_passthrough", "True Passthrough (no RAW auth)"],
