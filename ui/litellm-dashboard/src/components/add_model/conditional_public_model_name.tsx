...
const publicNameTooltipContent = (
  <div className="flex flex-col gap-2 text-left font-normal">
    <div>The name you specify in your API calls to RAW Proxy</div>
    <div>
      <strong>Example:</strong> If you name your public model <code className={tooltipCodeClassName}>example-name</code>
      , and choose <code className={tooltipCodeClassName}>openai/qwen-plus-latest</code> as the RAW model
    </div>
    <div>
      <strong>Usage:</strong> You make an API call to the RAW proxy with{" "}
      <code className={tooltipCodeClassName}>model = &quot;example-name&quot;</code>
    </div>
    <div>
      <strong>Result:</strong> RAW sends <code className={tooltipCodeClassName}>qwen-plus-latest</code> to the provider
    </div>
  </div>
);
...
const columns: ColumnDef<ModelMapping>[] = [
  ...
  },
  {
    ...
    header: () => (
      <span className="flex items-center">
        RAW Model Name
        <SimpleTooltip content={<div>The model name RAW will send to the LLM API</div>} width="360px" />
      </span>
    ),
  },
];
  ...
  return (
    <MountedFormField
      ...
      label={
        <span className="flex items-center">
          Model Mappings
          <SimpleTooltip content="Map public model names to RAW model names for load balancing" />
        </span>
      }
      required
