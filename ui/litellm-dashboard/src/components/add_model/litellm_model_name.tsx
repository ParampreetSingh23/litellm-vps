...
const LiteLLMModelNameField: React.FC<LiteLLMModelNameFieldProps> = ({
  ...
  return (
    <>
      <MountedFormField
        name="model"
        label={labelWithHint("RAW Model Name(s)", "The model name RAW will send to the LLM API")}
        required
        rules={{
          validate: {
      ...
      <div className="grid grid-cols-24">
        <p className="col-start-11 col-span-14 text-sm mb-3 mt-1">
          {selectedProvider === Providers.Azure
            ? "Your deployment name will be saved as the public model name, and RAW will use 'azure/deployment-name' internally"
            : "The model name RAW will send to the LLM API"}
        </p>
      </div>
    </>
  );
