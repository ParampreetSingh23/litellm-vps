...
describe("LogDetailContent", () => {
  ...
  it("should display RAW Overhead when litellm_overhead_time_ms is in metadata", () => {
    render(
      <LogDetailContent
        logEntry={createLogEntry({
          metadata: {
    ...
    expect(screen.getByText("RAW Overhead")).toBeInTheDocument();
    expect(screen.getByText("42.50 ms")).toBeInTheDocument();
  });

  it("should not display RAW Overhead when litellm_overhead_time_ms is absent from metadata", () => {
    render(<LogDetailContent logEntry={createLogEntry({ metadata: { status: "success" } })} />);

    expect(screen.queryByText("RAW Overhead")).not.toBeInTheDocument();
  });
