...
const RESPONSE_CACHE_TOOLTIP =
  "Whether this request was served from RAW's response cache (e.g. Redis / in-memory), skipping the LLM provider call entirely. This is separate from provider prompt caching; a Miss here does not mean prompt caching failed.";
const CACHE_KEY_TOOLTIP =
  "The key RAW computed for this request in the response cache. Requests with the same cache key share a cached response; a different key means the request content did not match any cached entry.";
      ...
      <Card size="sm" style={{ marginBottom: 0 }}>
        ...
        <CardContent>
          <DescriptionList>
            ...
            {showResponseCache && (
              <DescriptionItem label={<MetricLabel label="Response Cache" tooltip={RESPONSE_CACHE_TOOLTIP} />}>
            ...
            {responseCacheKey && (
              <DescriptionItem label={<MetricLabel label="Cache Key" tooltip={CACHE_KEY_TOOLTIP} />}>
            ...
            {metadata?.litellm_overhead_time_ms !== undefined && metadata.litellm_overhead_time_ms !== null && (
              <DescriptionItem label="RAW Overhead">{metadata.litellm_overhead_time_ms.toFixed(2)} ms</DescriptionItem>
            )}
