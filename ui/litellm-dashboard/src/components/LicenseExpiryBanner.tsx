...
const expiryDescription = (tier: "warning" | "critical" | "expired"): React.ReactNode => {
  if (tier === "expired") {
    return <>Enterprise features are now disabled. Contact your administrator to restore access</>;
  }
  if (tier === "critical") {
    return <>Renew now to avoid losing enterprise features. Contact your administrator</>;
  }
  return <>Renew before it lapses to keep enterprise features. Contact your administrator</>;
};
...
export const LicenseExpiryBannerView: React.FC<LicenseExpiryBannerViewProps> = ({ licenseInfo }) => {
  ...
  const message =
    tier === "expired"
      ? `Your RAW Enterprise license expired on ${formattedDate}`
      : `Your RAW Enterprise license ${describeCountdown(days)} (${formattedDate})`;
