...
async function selectModel() {
  ...
  await user.click(await screen.findByRole("combobox", { name: "RAW Assistant model" }));
  await user.click(await screen.findByRole("option", { name: "chat-model" }));
}

async function openWidget() {
  fireEvent.click(await screen.findByRole("button", { name: "RAW Assistant" }));
  await selectModel();
}

function send(text: string) {
  fireEvent.change(screen.getByPlaceholderText("Ask RAW Assistant…"), { target: { value: text } });
...
describe("RAW Assistant in the gateway", () => {
  it.each([
    ["sidebar", SidebarAccountMenu],
    ["navbar", UserDropdown],
  ] as const)("persists Hide RAW Assistant from the %s account menu", async (_name, Menu) => {
    gateway([]);
    const user = userEvent.setup();
    const view = renderWidget(Menu);
    await screen.findByRole("button", { name: "RAW Assistant" });
    await user.click(screen.getByRole("button", { name: /account menu/i }));
    const toggle = await screen.findByRole("switch", { name: "Toggle hide RAW Assistant" });
    expect(toggle).not.toBeChecked();
    await user.click(toggle);
    expect(toggle).toBeChecked();
    expect(screen.queryByRole("button", { name: "RAW Assistant" })).not.toBeInTheDocument();
    ...
    expect(screen.queryByRole("button", { name: "RAW Assistant" })).not.toBeInTheDocument();
    await user.click(screen.getByRole("button", { name: /account menu/i }));
    const savedToggle = await screen.findByRole("switch", { name: "Toggle hide RAW Assistant" });
    expect(savedToggle).toBeChecked();
    await user.click(savedToggle);
    expect(await screen.findByRole("button", { name: "RAW Assistant" })).toBeInTheDocument();
  });

  it("isolates Hide RAW Assistant by admin and gateway and reacts to another tab clearing it", async () => {
    gateway([]);
    const user = userEvent.setup();
    const view = renderWidget(SidebarAccountMenu);
    await screen.findByRole("button", { name: "RAW Assistant" });
    await user.click(screen.getByRole("button", { name: /account menu/i }));
    await user.click(await screen.findByRole("switch", { name: "Toggle hide RAW Assistant" }));

    session("proxy_admin", "second-admin");
    view.refresh();
    expect(await screen.findByRole("button", { name: "RAW Assistant" })).toBeInTheDocument();
    expect(screen.getByRole("switch", { name: "Toggle hide RAW Assistant" })).not.toBeChecked();
    session();
    view.refresh();
    expect(screen.queryByRole("button", { name: "RAW Assistant" })).not.toBeInTheDocument();
  ...
  it.each([
    ["sidebar", SidebarAccountMenu],
    ["navbar", UserDropdown],
  ] as const)("does not offer Hide RAW Assistant to a view-only admin in the %s menu", async (_name, Menu) => {
    ...
    expect(screen.queryByRole("switch", { name: "Toggle hide RAW Assistant" })).not.toBeInTheDocument();
  });
  ...
  it("uses the chosen model and existing session header for reads", async () => {
    ...
    renderWidget();
    fireEvent.click(await screen.findByRole("button", { name: "RAW Assistant" }));
    expect(await screen.findByPlaceholderText("Ask RAW Assistant…")).toBeDisabled();
    await selectModel();
  ...
  it("shows image descriptions without loading remote images from an admin answer", async () => {
    ...
    expect(within(screen.getByLabelText("RAW Assistant conversation")).queryByRole("img")).not.toBeInTheDocument();
  });
  ...
  it("keeps a single inline action through review, close/reopen and one confirmed write", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Close RAW Assistant" }));
    fireEvent.click(screen.getByRole("button", { name: "RAW Assistant" }));
    ...
    expect(screen.getByLabelText("RAW Assistant conversation")).toHaveTextContent(
  ...
  it("waits for fresh settings and consent after switching workers with the same admin session", async () => {
    ...
    expect(screen.queryByPlaceholderText("Ask RAW Assistant…")).not.toBeInTheDocument();
  ...
  it("leaves an oversized draft editable and keeps it out of the transcript", async () => {
    ...
    send(draft);
    expect(screen.getByPlaceholderText("Ask RAW Assistant…")).toBeEnabled();
    expect(screen.getByPlaceholderText("Ask RAW Assistant…")).toHaveValue(draft);
    expect(screen.getByRole("button", { name: "Send message" })).toBeDisabled();
    expect(screen.getByRole("alert")).toHaveTextContent("8,000 characters");
    expect(within(screen.getByLabelText("RAW Assistant conversation")).queryByText(draft)).not.toBeInTheDocument();
  });
