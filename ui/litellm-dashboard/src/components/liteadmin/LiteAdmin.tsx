          ...
          <Card size="sm">
            ...
            <CardContent className="space-y-3">
              ...
              <p className="text-muted-foreground">
                This gateway uses a different address. RAW Assistant will send your existing session credential to the
                address shown above.
              </p>
            </CardContent>
            <CardFooter>
    ...
    >
      ...
      <div className="space-y-3 border-t p-3">
        {models.isPending ? (
          ...
          <SearchSelect
            aria-label="RAW Assistant model"
        ...
        {models.isSuccess && models.data.length === 0 && (
          <Alert role="status">
            <AlertDescription>Add a chat model to your gateway to use RAW Assistant.</AlertDescription>
          </Alert>
        )}
...
function PanelHeader({ close, children }: { close: () => void; children?: ReactNode }) {
  return (
    <div className="flex items-start justify-between gap-3 border-b p-4">
      <PopoverHeader>
        <PopoverTitle>RAW Assistant</PopoverTitle>
        <PopoverDescription>Ask about your gateway. Review changes in chat.</PopoverDescription>
      </PopoverHeader>
      <div className="flex shrink-0">
        {children}
        <Button variant="ghost" size="icon-sm" aria-label="Close RAW Assistant" onClick={close}>
          <X className="size-4" />
        </Button>
      </div>
    </div>
