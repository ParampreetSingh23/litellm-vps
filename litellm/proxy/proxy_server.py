...
ui_message = f"👉 [```RAW Admin Panel on /ui```]({ui_link}). Create, Edit Keys with SSO. Having issues? Try [```Fallback Login```]({fallback_login_link})"
ui_message += f"\n\n🔎 [```RAW Model Hub```]({model_hub_link}). See available models on the proxy."


### CUSTOM BRANDING [ENTERPRISE FEATURE] ###
_title: Final = os.getenv("DOCS_TITLE", "RAW API") if premium_user else "RAW API"
_description: Final = (
    os.getenv(
        "DOCS_DESCRIPTION",
        f"Enterprise Edition \n\nRAW by Rabbitt: OpenAI-compatible gateway for 100+ LLMs.\n\n{ui_message}",
    )
    if premium_user
    else f"RAW by Rabbitt: OpenAI-compatible gateway for 100+ LLMs.\n\n{ui_message}"
)
